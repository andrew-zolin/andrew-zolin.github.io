...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (videoBackground) {
        // Check if device is mobile or prefers reduced data
        ...
        if (!isMobile && !prefersReducedData) {
            ...
            const videoObserver = new IntersectionObserver((entries) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        ...
                        videoObserver.unobserve(videoBackground);
                    }
                });
            }, { threshold: 0.1 });
            ...
            // Hide video on mobile or reduced data
            videoBackground.style.display = 'none';
        }
    }

    // Optimize partner logos loading
...
document.querySelectorAll('.fade-in, .service-box, .process-step').forEach(el => {
